...
3. **Every optional package surface is off by default.** No `DartPadAnalyzer` (local analyzer only), no error gutter, no package find UI (its strings are unlocalizable English, which would breach FR-I18N), and the `wip/`-quality autocomplete popup is enabled only behind an explicit, tested opt-in. A CI grep gate forbids `DartPadAnalyzer` and `package:http` in `app/lib/`, in the same spirit as the existing no-leak and import-boundary gates — so the dormant network capability stays dormant by construction, not by intention.
...
- **Dormant `DartPadAnalyzer` network path.** *Mitigation:* default local analyzer, plus a CI gate forbidding it and `package:http` in `app/lib/`.
