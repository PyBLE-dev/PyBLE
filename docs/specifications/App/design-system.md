...
- **Contents** (row, `SignalSpacing.sm` gap, `EdgeInsets.symmetric(horizontal: SignalSpacing.md, vertical: SignalSpacing.xs)`): the state glyph + the **localized state label** in **`colorScheme.onSurface`** (`labelMedium`, scaler ≤ 1.3). On `ready`/`running`, once the current connection session's `DeviceInfo` is available, a localized compact pattern appends its verbatim board ID + PyBLE agent version (English: `Ready · 5646 · 0.6.0`); empty values use the localized **Not reported** fallback. A late result from a detached session is never rendered. The label uses `onSurface` (not the state color) so text is AA-safe at any hue (§1.3). The full **state word never ellipsizes** — under width pressure only the appended identity summary may ellipsize, and the pill's minimum content remains glyph + full word (§7.1 shrink order).
...
- **Normal action toolbar:** ordinary Files actions occupy exactly one horizontal action row below the path; they MUST NOT wrap into a second icon row, scroll horizontally, clip, or shrink below the 48 dp target. The localized, tooltip-labelled Select-files and GitHub-import actions remain directly visible in a constrained landscape pane. Refresh, New file, New folder, and Upload move together into one localized, keyboard- and screen-reader-reachable labelled overflow when all six direct actions do not fit. A roomy Files surface exposes all six actions directly and omits the overflow. Overflow items retain the same enabled state and invoke the same action as their direct counterparts.
- **Multi-select contextual bar:** on multi-select, the §7.4 pane header is replaced by a bar showing the selected count (`titleMedium`) + Cancel, Select-all-shown, and bulk Delete actions; destructive bulk delete uses `colorScheme.error` (§7.8); selected rows use the §7.10 `surfaceContainerHigh` selection fill. The current `cwd` remains visible as one ellipsized `SignalType.code` line. In a narrow Files pane or at 2× text the bar grows to two rows; it never shrinks a target below 48 dp or clips the count/actions.
- **Selection eligibility:** the first FR-FILES-5 increment selects regular files only. Folders remain visible but do not gain a checkbox; locked §7.10 control-plane/scratch rows are excluded. “Select all shown” means every eligible file present in the current rendered listing, never descendants or entries omitted by `FILE_LIST more == 1`.
- **Bulk confirmation and result:** one scrollable destructive confirmation shows the exact folder and every ordered filename, with Cancel focused before Delete. During sequential deletion the contextual bar stays locked and shows item progress. A complete result exits selection; a failed/partial live-region result keeps only unresolved names selected and states how many were deleted and remain. Neither copy nor color may imply atomic rollback.
