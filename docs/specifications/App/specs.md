...
- **FR-PBLE-4** — The client MUST complete a pending `CMD` only from `TYPE=RSP` with the exact originating `{OPCODE, ID}` pair. A wrong-opcode `RSP` with the same reused 1-byte `ID`, or any non-`RSP` frame with a nonzero `ID`, MUST NOT complete the pending command or consume, replace, or hide its matching response in either arrival order. Correlation is connection-local and retains the one FR-PBLE-16 absolute arrival deadline; unrelated frames do not extend it. The client MUST route valid `EVT` frames (`ID = 0`) to the appropriate stream (console, run-state, file-transfer acks/data). MUST (*source: protocol.md §3.1, §4; verify: conformance; story: A-13*)
...
- **FR-FILES-5** — The explorer MUST support a multi-select bulk-delete workflow for eligible regular files shown in one current board folder. Selection MUST be session- and folder-bound; deletion MUST be confirmed once, execute sequentially, stop on the first failure/session change, refresh once, and report exact succeeded, failed/current, and unattempted paths without claiming rollback or transactionality. MUST (*source: PRD §9.2/§21.1, [ADR-0043](../../decisions/0043-session-bound-visible-file-multi-delete.md); verify: unit/widget/golden/HIL; story: A-30*)
- **FR-FILES-6** — The explorer MUST distinguish the user workspace (`/main.py`, `/lib/*.py`, `/data/*`) from the agent control plane and transfer scratch paths, which it MUST NOT present as editable or bulk-selectable. Relative to board-reported `fs_root`, lowercase top-level `pyble*`/`pble*`, exact top-level `boot.py`/`_boot.py`, and any `.pbltmp`-suffix component are locked with the same case-sensitive predicate as the PBLE/1 workspace jail; ordinary nested `pyble*` basenames remain editable. MUST (*source: PRD §9.2, §10.4, firmware.md §1, protocol.md §5; verify: unit/widget; story: A-30*)
...
**Working-loop increment — FROZEN (A-30 subset · `[docs]` 2026-07-02, [ADR-0010](../../decisions/0010-working-loop-in-memory-document.md), [TDD §11.6](TDD.md#116-working-loop-provider-contract-frozen)).** In scope: `listDir` rooted at `fs_root` with into/up navigation, open-in-editor (`getFile` → editor buffer), upload-current-buffer (`putFile`), mkdir, delete, rename, `newFile`, continuous transfer progress + whole-file CRC success, typed-error → localized message, disabled-with-guidance while disconnected (FR-FILES-1/2/3/4/8, binding via the `Connection` seam only, CON-8). **Deferred:** export-to-device-storage **Download** (needs local persistence + a new dep — open-in-editor is the read-into-app path this increment). `.py`/data-only holds (FR-FILES-7, CON-3).

**Visible-file multi-delete — FROZEN (`[docs]` 2026-08-27, [ADR-0043](../../decisions/0043-session-bound-visible-file-multi-delete.md), [TDD §11.6](TDD.md#116-working-loop-provider-contract-frozen)).** A localized tooltip/semantics-labelled Select action remains directly visible when the normal Files toolbar compacts into one action row; Refresh, New file, New folder, and Upload move into one labelled overflow while GitHub import also remains direct. The Select action and eligible-file long press enter a contextual mode scoped to the exact shown direct regular files in one `cwd`; folders, descendants, firmware-reserved/control-plane paths, transfer scratch paths, and omitted/truncated entries are never selected. Select all means all eligible **shown files**. Tap/Space toggles instead of opening; normal navigation and mutation affordances are unavailable until Cancel/Escape/Back. One confirmation displays the exact folder and ordered filenames. The app then validates the captured `cwd`, paths, and opaque connection-session stamp, awaits ordinary `Connection.delete` calls sequentially, stops before the next path on the first failure/session change, never rolls back or retries, and refreshes the captured folder once only if the same session remains current. Complete, failed, and partial results truthfully identify succeeded, failed/current, and unattempted paths; still-rendered unresolved files remain selected, while reconciliation never creates a selected ghost for an absent path. Navigation that hides Files clears selection without cancelling an already-issued board mutation. Folders retain the existing one-at-a-time empty-directory delete and no recursive delete exists.
...
- **FR-EDIT-5** — Touch targets MUST be tablet-friendly and the editor MUST remain usable on a phone without layout breakage. The editor MUST expose a bounded, editor-only code-font adjustment that composes with (and never replaces or clamps) the platform text scaler; line numbers MUST track the selected code size. MUST (*source: PRD §9.3, §13.2, §13.7; verify: widget/golden; story: A-20*)
...
**Working-loop increment — FROZEN (A-20 subset · `[docs]` 2026-07-02, amended 2026-08-22 by [ADR-0012](../../decisions/0012-flutter-code-editor-behind-editorsurface.md), [ADR-0010](../../decisions/0010-working-loop-in-memory-document.md), [TDD §11.1](TDD.md#111-editor)/[§11.6](TDD.md#116-working-loop-provider-contract-frozen); run path superseded 2026-07-27 by [ADR-0013](../../decisions/0013-clean-room-blockly-one-way-file-backed.md)).** In scope: the ADR-0012 `EditorSurface` seam with `flutter_code_editor` 0.3.5 as the default rich adapter and the existing plain `TextField` as a live fallback; Python highlighting, always-visible one-based line numbers, safe auto-indent/Tab handling, and an editor-only 10–24 sp font adjustment in one-point steps (default 14 sp). Code and gutter resize together, platform text scaling still applies, and resizing MUST NOT modify the document, selection, dirty state, or board path. The size is app-session state, independent of file and board, until A-24's single Drift `SettingsStore` makes it durable; a second preferences store MUST NOT be introduced. The single volatile in-memory current document (name/content/dirty/boardPath), **New**, and board-backed Save/Run behavior remain as frozen by ADR-0010/0013. Save and Run capture one normalized document snapshot; transfer completion marks it saved/bound only when the live document still equals that snapshot, so mid-transfer edits or a replacement document remain dirty. The editor binds to `Connection` only (FR-EDIT-6, CON-8), and editor source MUST NOT reach a network analyzer. **Still deferred to A-24 or later:** multi-file tabs, app-owned localized find/replace, the remaining external-keyboard shortcuts, durable display settings, Save-to-project, and buffers-survive-restart. The document provider's `openFromBoard`/`markSaved` and the session display provider are the A-24 persistence hooks (RECORDED deviations, ADR-0010/0012).
...
- **FR-CONNECT-7** — Ordinary Disconnect, replacement of the selected board, and disposal of an owned connection MUST close that connection's actual BLE link, not merely detach the UI. Teardown MUST be idempotent, abort pending protocol/transfer work and release owned subscriptions; a close failure MUST remain an error, not a successful physical-disconnect claim. A superseded or cancelled connection attempt MUST NOT publish its late result or disturb a newer session, and MUST close any link it eventually acquires. Direct engine-backed connections MUST NOT acquire ownership of externally supplied transports. MUST (*source: FR-BLE-4/5, FR-CONN-5/7, SEC-2; verify: actual protocol engine with recording fake links, manager races, real iPadOS/Android disconnect/reconnect HIL*)
...
- **FR-ABOUT-8** — Opening About, Open-source licenses, or Privacy policy MUST preserve the current editor/Blocks document, selected surface, console state, and live board session. MUST (*source: PRD §13.1, §19; verify: widget/integration; story: X-11*)
- **FR-ABOUT-9 — Offline privacy policy (`[docs]` 2026-09-10).** About's privacy card MUST expose a clearly labelled, ≥48 dp **Privacy policy** action opening a full offline route, available without a board or Internet connection. Its ARB-sourced policy MUST identify PyBLE and its maintainer, show the effective date, and reproduce the disclosures at `https://pyble.dev/privacy`: no account/advertising/analytics/telemetry/crash reporting/payment/cloud service; the sole optional public GitHub API workflow and request metadata; direct user-started board transfers; the absence of guaranteed BLE pairing/encryption; locally processed board discovery data and legacy Android location permission; publicly advertised board labels; distinct app, board, export, and operating-system-backup retention/deletion; third-party components/platform services; website infrastructure handling; policy changes; and the current verified privacy contact `viwat.v@chula.ac.th`. The page MUST display the canonical policy URL as selectable text, perform no network request or board verb, and inherit FR-ABOUT-7's responsive/accessibility requirements. Back MUST return to About and the prior IDE session unchanged. A future policy change MUST update the public and bundled disclosures together. MUST (*source: [website §5](../website.md#5-privacy-contract), [Google Play User Data policy](https://support.google.com/googleplay/android-developer/answer/10144311?hl=en); verify: widget/locale/integration; story: X-11*)
...
- **BLD-3** — The app MUST follow SemVer; v1.0 is the first production release and a breaking change bumps MAJOR. MUST (*source: PRD §18.1; verify: unit; story: X-11*)
  - **App 0.2.0 beta train — FROZEN (`[docs]` 2026-08-27, [ADR-0044](../../decisions/0044-start-app-v020-beta-train-at-build-5.md)).** The current compatible Unreleased feature set advances the app base version to `0.2.0`; its first candidate uses globally monotonic Flutter build number `5`. `app/pubspec.yaml` owns `0.2.0+5`; an automated unit contract MUST keep that base synchronized with the compile-time PBLE/1 HELLO identity and the exact public GitHub User-Agent. This candidate designation does not change firmware `0.6.0`, PBLE/1, the store-parity rule, or the qualification required before calling an app build released. Version `1.0.0` remains reserved for the first production release.
...
- **BLD-13 — iOS/iPadOS 15 deployment floor (FROZEN · X-11 · `[docs]` 2026-08-27, [ADR-0045](../../decisions/0045-adopt-ios-15-deployment-floor.md)).** Every authored Runner Xcode build configuration MUST set `IPHONEOS_DEPLOYMENT_TARGET` exactly to `15.0`. Every later exported top-level application `MinimumOSVersion` and main Mach-O build-version command MUST encode an iOS minimum of at least `15.0`; the exported-IPA gate MUST reject any regression. A bundled dependency may retain a lower compatible minimum without lowering the containing application's installation floor. A floor change requires a clean source rebuild/archive and MUST NOT patch or merely re-export an older signed artifact. The retained `0.2.0 (5)` iOS/iPadOS 13 TestFlight handoff remains immutable; this source-only change creates or uploads no build 6 and changes no Android, PBLE/1, or firmware baseline. MUST (*source: PRD §18.2; Apple `MinimumOSVersion`; verify: unit/build/manual; story: X-11*)
...
- **OI-1 — Flutter Python-editor widget choice. CLOSED (2026-07-25) by [ADR-0012](../../decisions/0012-flutter-code-editor-behind-editorsurface.md).** `flutter_code_editor` is pinned exactly at 0.3.5 behind the app-owned `EditorSurface` seam, with optional network/error/find/autocomplete surfaces disabled, adapter-owned safe Tab handling, and the plain `TextField` retained as a live fallback. Tablet maturity remains the bounded R1 risk in [TDD §17](TDD.md#17-risks--open-questions), with explicit fallback triggers rather than an open widget decision.
