...
**Public API:** an app-owned `EditorSurface` push-in/stream-out widget contract: the caller supplies the current text, change callback, editor text style, localized hint, and Signal colors. The default factory builds the rich `flutter_code_editor` adapter; tests and the R1 escape hatch can select the retained plain-field adapter without changing feature code. A later `EditorTabs` controller is backed by Drift `project_files`.

**Key state:** the volatile current buffer (dirty flag) plus session-wide editor display settings. Font size defaults to 14 sp, moves in one-point steps over the inclusive 10–24 sp range, and applies only to code plus its line-number gutter; the platform `TextScaler` still composes over it. A-24 hydrates that display value through the single Drift settings store. Open buffers, active tab, and find/replace state land with the full persistent editor. Save persists to Drift (D4); Run uploads then runs through `Connection` (the §8.2 loop).
...
- **Connect controller + view (`lib/connect`).** A `ConnectController` (Riverpod `Notifier`, ADR-0007) projects the manager into an immutable `ConnectState { ConnectPhase phase, BleReadiness readiness, List<ScanHit> hits, ScanHit? selected, DeviceInfo? deviceInfo, Object? error }` (with `int get hitCount`), exposing `startScan`/`stopScan`/`connect(id)`/`disconnect`. On reaching `connected` it reads `connection.deviceInfo()` and stores it — the **DeviceInfo proof** the UI shows. `ConnectScreen` (the surface, hosted by `lib/app/pages/connect_page.dart` as a thin delegator) renders: the readiness banner (adapter-off / unauthorized / unsupported with localized rationale, FR-CONNECT-3); the scan CTA → live `ScanHit` list (name + RSSI, FR-CONNECT-1) → connecting indicator → connected `DeviceInfo` card (board ID / PyBLE firmware / chip / mpy / free-mem, FR-CONNECT-2) → disconnect; the unsupported-`proto_version` update prompt (FR-CONNECT-6); and a **live diagnostics panel** (adapter state, scanning yes/no, hit count, last-error detail) for on-hardware debugging (FR-CONN-6). The board-ID row reads only `DeviceInfo.deviceId` (never `ScanHit.id`); board ID and agent version are verbatim technical values. Empty compatibility defaults render localized **Not reported** rows and do not gate the connection. Because ADR-0011 replaces the Connect surface with the IDE at `ready`, the connected shell's `ConnectionStatusPill` also watches `connectControllerProvider`; this keeps the auto-disposed controller alive, defensively fetches `deviceInfo()` for an already-attached board, and appends its board ID + agent version to the state signal. Each fetch captures `connectionSessionStampOf(manager.connection)` and may publish its result only while that opaque local stamp is still current; a late prior-session result is discarded, and a replacement session may fetch concurrently rather than being suppressed by the old request. The pill shows state alone while the current-session fetch is pending, preserves the full state word under width pressure, and exposes the complete labeled summary through semantics.
...
PBLE/1's 128-byte UTF-8 ceiling. The action captures the board-reported
`fsRoot` alongside `capturedCwd`; presentation shows localized root-destination
guidance whenever those canonical paths are equal. Target derivation also
mirrors the PBLE/1 filesystem jail's protected top-level names. At the captured
root, a basename beginning with lowercase `pyble` or `pble`, or exactly equal
to `boot.py` or `_boot.py`, raises a sanitized `protectedRootTarget` failure
...
`flutter_code_editor` 0.3.5 behind the app-owned `EditorSurface` interface ([§4.3](#43-libeditor--code-editor), [ADR-0012](../../decisions/0012-flutter-code-editor-behind-editorsurface.md)), with the existing plain `TextField` retained as the immediate fallback and a WebView-hosted editor (Monaco/CodeMirror) remaining the documented later fallback ([PRD §16.1](../prd.md), [OI-1](specs.md)). Python highlighting, line numbers, auto-indent (FR-EDIT-1); tabs + Save/Run (FR-EDIT-2); find, optional find/replace (FR-EDIT-3); external-keyboard shortcuts for Save/Run/Stop/comment/find (FR-EDIT-4); large touch targets, no phone breakage, and editor-only font adjustment that preserves system scaling (FR-EDIT-5). Save persists to Drift; Run uploads then runs through `Connection` (FR-EDIT-7).

**Working-loop increment — FROZEN (A-20 subset · `[docs]` 2026-07-02, amended 2026-08-22 by [ADR-0012](../../decisions/0012-flutter-code-editor-behind-editorsurface.md), [ADR-0010](../../decisions/0010-working-loop-in-memory-document.md)).** The shippable editor keeps the **single volatile in-memory current document** ([§11.6](#116-working-loop-provider-contract-frozen)) while moving its presentation behind `EditorSurface`. The default rich adapter supplies Python highlighting and an always-visible, one-based, vertically synchronized line-number gutter; only line numbers are enabled in the gutter (package errors and folding handles stay off), package find/autocomplete UI stays off, and no network analyzer is reachable from `lib/editor`. The plain `TextField` remains executable as the R1 fallback. Both paths preserve the smart-punctuation invariant, and adapter-owned Tab handling must indent a multi-line selection without replacing it.

The same bounded increment adds session-wide editor zoom: default 14 sp, inclusive 10–24 sp, one-point steps. Code and gutter share the selected family/size/height and update immediately; changing it preserves document bytes, selection, undo history, dirty state, and board path. Controls remain accessible at phone width and compose with the full platform text scaler. Durable display preference, syntax-editor tabs, app-owned localized find/replace, the remaining shortcuts, Save-to-project (FR-EDIT-7), and buffers-survive-restart (NFR-REL-3/FR-PROJ-6) remain carried to A-24 or later. The document and display providers are the hooks A-24 rebinds to the one Drift `SettingsStore`; no parallel preferences backend is admitted.
...
**Visible regular-file multi-delete — FROZEN (`[docs]` 2026-08-27, [ADR-0043](../../decisions/0043-session-bound-visible-file-multi-delete.md)).** `FilesView` owns a temporary selection of eligible shown direct regular-file names plus captured `cwd` and opaque session stamp. A localized tooltip/semantics-labelled Select icon remains directly visible in both compact and roomy normal toolbars; it or an eligible-row long press enters selection. The compact normal toolbar keeps GitHub import direct and moves Refresh, New file, New folder, and Upload into one labelled overflow so normal action icons remain exactly one row. Rows become checked toggles, Select all includes eligible shown files only, and Cancel/Escape/Back exits. Folders and locked entries never enter the set. The contextual bar replaces normal Files actions and uses a live selected count, destructive Delete, and item progress. One scrollable dialog presents the exact `cwd` and ordered names with Cancel focused before Delete.

`deleteMany` snapshots the controller's displayed order, rejects duplicate/unknown/directory/unsafe/reserved/overlength names and a mismatched expected `cwd` or session before the first `Connection.delete`, then awaits each delete serially. A second valid batch receives a local `busy` failure, reports all of its ordered paths unattempted, and sends no verb. On the first typed error or session change it starts no later delete and returns exact succeeded/current/unattempted accounting. It performs no rollback or automatic retry. If at least one delete was issued, it calls `listDir(capturedCwd)` exactly once only while the captured session is current; a primary delete error wins over a reconciliation error. Complete success clears selection; failed/partial removes succeeded names, keeps still-rendered unresolved names selected, focuses and announces one localized result, and leaves the refreshed listing visible. Reconciliation cannot create a selected ghost for an absent failed path, and a later selection edit dismisses rather than mutates prior result accounting. Navigation that hides Files clears local selection even if an already-issued, non-cancellable board delete must finish; switching Editor/Console centre focus while the landscape Files sidebar remains visible does not hide it. Folders retain the existing single empty-directory delete; there is no recursion or new PBLE opcode.
...
| `lib/github_import/` | canonical URL/host parser; default/explicit ref→full-SHA pin; lazy response models and regular-`.py` filter; safe basename target derivation with protected-root rejection; strict UTF-8/NUL and 256 KiB/file + 1 MiB/batch bounds; rate/error mapping; operation generations; result accounting | fake-GitHub + FakeConnection resolve/browse/select/review; exact target preview; actionable protected-root guidance with zero board I/O; overwrite consent/recheck; all fetches before first PUT; stable sequential PUT order; pre/during-commit cancellation; session swap; partial truth + refresh; no mkdir/open/run; keyboard/focus/semantics | compact sheet + wide dialog in portrait/landscape; loading, empty, validation, rate-limit, review, overwrite, upload, partial, and complete states; 2× text, high contrast, keyboard inset | — | deterministic mock-HTTP→FakeConnection flow; pinned public fixture→disposable child directory with exact bytes and no execution (HIL) | every field/action/loading/error/rate/overwrite/progress/result/semantics label |
...
- **R1 — Flutter Python-editor widget residual risk (OI-1 closed).** ADR-0012's evaluation found a dormant upstream, superlinear whole-buffer work, and an unsafe upstream Tab binding. PyBLE pins 0.3.5 behind `EditorSurface`, owns Tab interception and smart-punctuation normalization, disables the optional network/error/find/autocomplete surfaces, and retains the plain-field implementation. Switch to that live fallback if target-iPad testing finds an adapter-irreparable correctness/data-loss defect, realistic beginner files become unusably slow, or a Flutter SDK upgrade leaves the package incompatible; a later WebView-hosted Monaco/CodeMirror surface can use the same feature seam ([§11.1](#111-editor)).
