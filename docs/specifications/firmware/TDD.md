...
>
> **Frozen v0.6.0 pre-publication replacement (2026-08-20, `[docs]`,
> [ADR-0037](../../decisions/0037-fixed-profile-reset-and-transfer-product-slos.md),
> [ADR-0038](../../decisions/0038-replace-unpublished-v060-candidate-in-place.md)):**
> the local `719b211…` candidate/tag was never public and is abandoned. Strict
> descendants of inclusive predecessor boundary `5620f2f…` retain version
> 0.6.0 but use ESP/Pico reset ceilings 3,000/7,000 ms and fixed PUT/GET floors
> 6,600 bytes/s. Old bytes/HIL cannot qualify the replacement.
>
...
pool and checks the exact incarnation and state: matching `RESERVED`/`READY`
means a stale wake and continues, exact `COMPLETE` returns, and an incarnation
mismatch cancels. This covers an old transition whose give is delayed across
hard recycle/reserve as well as a new completion that fills the binary
...
the TX mutex while it detaches runner/console worker pointers, sets
`MP_STATE_VM(pble_runner_sysexit)` and `MP_STATE_VM(pble_fs_put_file)` to
`MP_OBJ_NULL`, and calls `__real_mp_thread_deinit`; it releases only after the
old VM tasks have been deleted. Therefore passed-gate callbacks cannot
dereference a force-deleted VM thread and a paced sender cannot reacquire TX
just before its owning task is deleted.
...
registrations, including defensively clearing both custom VM roots again. A
queued response callout is a static kick with no captured frame pointer. It
enters lifecycle activity, does nothing while reset/not-ready is closed, and
otherwise peeks the current ticket/incarnation/session under lock; a callback
paused after that peek keeps reset from recycling state until it leaves. Reset
...
**Execution model:** a `RUN` while `state == running` returns `EBUSY` (FR-RUN-4). The ESP handler validates the request and initialized hand-off semaphore, remembers the exact prior runnable state (`idle`, `done`, or `error`), makes a provisional, non-observable reservation of `running`, and copies the request. It then encodes the matching one-fragment `RSP{OK}` in call-local storage and makes one connection-bound control submission attempt. That transport seam declares the specialized response pending, may wait under one absolute 15 ms deadline only for the current complete-message TX-mutex boundary, rechecks the originating connection after acquisition, and makes exactly one local Notify submission. It never waits or retries for drain, mbuf, or controller capacity on the NimBLE host task, and the pending gate prevents a later ordinary/bulk message from beginning before the attempt releases its boundary. `PBLE_TX_OK` is the admission cut: only then does the provisional reservation become an observable lifecycle transition, the handler give the binary runner semaphore exactly once, and the handler suppress the dispatcher's generic response. The empty-semaphore give is an invariant of the single `running` reservation. Boundary-deadline expiry, no connection, connection mismatch, or local Notify failure restores the remembered state, suppresses generic response fallback, and returns without a worker wake, execution, console output, or RUN event. A disconnect after `PBLE_TX_OK` does not revoke the accepted run. The non-dispatch auto-run path remains a direct reserve/copy/give with no response.
...
defines allocation-free, idempotent `__wrap_mp_thread_deinit`. Under one
authoritative lifecycle lock, every complete CMD enters only while open and
...
worker pointers, clears both custom VM roots, and calls
`__real_mp_thread_deinit`; it releases TX only after that call deletes the old
tasks and returns. This closes the manual Ctrl-D and passed-gate
RUN/STOP/SOFT/CONSOLE/FS gaps before upstream force-deletes secondary tasks. Each PyBLE ESP board
...
remove an event already queued on the host eventq, the static callback carries
no retained frame pointer: it enters lifecycle activity, does nothing while
closed/not ready, and otherwise freshly peeks exact VM epoch, connection
generation, slot incarnation, and frame under lock. A callback paused after
that peek prevents reset recycle until it leaves. Reset then synchronizes with
response ownership as needed, hard-recycles every slot/incarnation and drains completion semaphores,
drains the FS queue and resets transfer state, and resets runner semaphore/RSM/
request buffers/worker pointer plus console buffers and BLE RX reassembly, and
defensively clears `MP_STATE_VM(pble_runner_sysexit)` and
`MP_STATE_VM(pble_fs_put_file)` again. It does not rely on unsafe callout
deinitialization or queued-event removal. Either lifecycle seam seeing an
already-`CLOSING` connection invokes `esp_restart()` immediately rather than
rotate it open or cancel its independent termination watchdog.
The soft-reboot timer stores its armed epoch and enters/leaves lifecycle
...
2. runs `assemble-oi1-baseline` against the immutable staged inputs and the
   ...
   under `docs/validation/firmware/oi1/`, derives exact image/headroom and heap
   thresholds, inserts the source-era fixed reset/goodput SLOs, and atomically
   updates the policy with its evidence SHA-256;
...
`source_commit` cannot select an era. Missing, equal-boundary, unrelated, or
unknown ancestry fails closed. The immutable `a8be631…` baseline remains the
input for static/heap derivation and diagnostic reset/goodput samples; it is
never rewritten.

Replacement v0.6.0 uses boot identifier
`fixed-profile-product-slo-esp3000-pico7000-v4` and goodput identifier
`fixed-product-slo-64k-under-10s-6600-v3`. Its exact fixed values are:

| Profile | Reset max (ms) | PUT min (B/s) | GET min (B/s) |
|---|---:|---:|---:|
...
All 10 reset and all five PUT/GET samples must pass without trim or retry.
Goodput 6,600 is `ceil_100(65536 / 10)` and implies the exact maximum passing
integer duration `9,929,696,969 ns`. Reset keeps the exact-service callback
endpoint, 15-second health timeout, and physical check. Transfer duration,
...
- `tests/firmware_tests/host/test_oi1_profile_bench.py`: fake BLE central and
  ...
  five-profile order; ESP/RP2 adapter discrimination; four-ESP 3,000 ms and
  Pico 7,000 ms exact-boundary acceptance plus one-ms crossing; all-ten reset
  enforcement; product-wide PUT/GET 6,600 acceptance plus one-byte/s crossing;
  all-five enforcement; rejection of swapped, arbitrary, baseline-fitted, or
  retry-replaced performance values; and preservation of exact integrity,
  reliability, physical, and link facts;
...
- `tests/firmware_tests/host/test_release_v060_contract.py` and
  `test_release_historical_source_contract.py`: exact V4/V3 derivation IDs,
  the five fixed policy rows, immutable `a8be631…` baseline bytes, V3/V2
  predecessor replay, strict-descendant replacement routing, rejection of the
  boundary itself plus unrelated/unknown ancestry, version-only routing
  rejection, and explicit rejection of `719b211…` candidate/HIL reuse;
...
**Requirements with no dedicated design element:** none. `SEC-9` is intentionally deferred (no v1.0 design surface beyond the additive-caps note in [§13](#13-security-design)). The `NFR-FP-*` measurement and enforcement design is frozen in §8.5; only static image/headroom and heap values remain evidence-derived for replacement v0.6.0, while reset/goodput are fixed product SLOs. Likewise the concrete label max-length, `SET_IDENTIFY_LED` encoding, and `IDENTIFY` blink-duration bound are owned by [protocol.md](../protocol.md) and tracked by OI-6, not invented here.
