...
- **FR-RUN-8** — `SOFT_REBOOT` MUST submit an observable `RSP{OK}` before it
  ...
  handler/console wiring is safe, and auto-run admission has completed. A
  VM-rooted marker cleared by `mp_init` MUST make new-VM detection exactly once;
  repeated initialization in one VM is idempotent, and any boot-wiring failure
