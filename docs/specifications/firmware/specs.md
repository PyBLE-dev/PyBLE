...
- **FR-RUN-8** — On the normal path where its pre-created grace timer arms
  ...
  pickup gate as STOP by publishing stop intent before timer arm. A worker whose
  reservation precedes that accepted response MUST perform no RUN event or user
  effect. Response-submission failure MUST reopen the provisional VM/FS gates
