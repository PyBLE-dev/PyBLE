...
- **FR-PROTO-6** — Every command MUST return the correct PBLE/1 1-byte status in its `RSP`, including error cases (`EBADREQ`, `ENOENT`, `EACCES`, `ENOSPC`, `EIO`, `ENOMEM`, `EBUSY`, `ECRC`, `ERANGE`, `EUNSUPPORTED`, `EINTERNAL`). The sole RUN-admission transport exception is FR-RUN-1: when the one local `RSP{OK}` submission attempt itself fails, that RUN is not admitted and MUST NOT make a fallback response attempt. — *(source: PRD §10.8, [protocol.md §8](../protocol.md#8-status--error-codes-1-byte-status-in-rsp); verify: conformance; story: F-02, P-04)*
