...
- **FR-PROTO-4** — The agent MUST correlate requests and responses by the 1-byte `ID`, echoing the request `ID` in the matching `RSP`; events MUST use `ID = 0`. A queued response MUST preserve its opcode and ID and remain bound to the originating connection generation; a reused numeric connection handle MUST NOT receive bytes from an earlier session. — *(source: [protocol.md §3.1](../protocol.md#3-framing); verify: conformance; story: F-02)*
...
- **FR-FS-3** — Download (`FILE_GET_BEGIN` / `FILE_GET_DATA` / `FILE_GET_END`) MUST stream a file from an offset and report a whole-file CRC for the app to verify. Its deferred queue item MUST carry a reserved response ticket with slot incarnation and originating connection generation, revalidate before any VFS side effect and again before publishing worker-scratch output, and complete the matching `RSP` before emitting dependent `FILE_GET_DATA`/`FILE_GET_END` events. Disconnect cancels/recycles the ticket; a late worker result MUST fail its incarnation check and no event or response byte may reach a later session. — *(source: PRD §8.4, §10.8, [protocol.md §5](../protocol.md#5-file-transfer-the-reliability-core); verify: unit, conformance, HIL; story: F-08)*
