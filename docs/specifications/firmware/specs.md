...
Status: **DRAFT (per-section freeze in effect)** · Owner: project maintainer · Last updated: 2026-08-14
...
- **FR-RUN-1** — `RUN { mode: file }` MUST execute a `.py` file from the workspace only after local Notify acceptance of its matching `RSP{OK}`, then emit `RUN_STATE(running)`. The ESP reference handler MUST reserve and copy first, attempt that connection-bound one-fragment response exactly once without waiting, and wake the runner exactly once only on `PBLE_TX_OK`. Mutex contention, no connection, a changed connection, or Notify backpressure MUST restore the exact prior runnable state and cause no response fallback, worker wake, execution, console output, or RUN event. — *(source: PRD §8.2, §10.6, [protocol.md §6](../protocol.md#6-run--stop--console); verify: unit, HIL, conformance; story: F-04)*
