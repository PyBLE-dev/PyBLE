...
**2026-08-19 §4 functional amendment:** the ESP native console stream exposes
...
- **FR-BLE-10** — The agent MUST implement PBLE/1 fragmentation/reassembly over RX/TX per [protocol.md §3.2](../protocol.md#3-framing). `FIRST` MUST abandon an incomplete receive run. Every RX-fragment host callback MUST make one non-blocking lifecycle-activity entry before reading/copying the fragment and remain counted through reassembly and any complete-CMD dispatch. Closed/not-ready refusal MUST atomically clear the incomplete run and drop the fragment. Admission and refusal reset MUST be one authoritative lifecycle-lock transaction: refusal clears only that same still-closed VM epoch before releasing the lock, so a paused refusal cannot later clear a fresh-epoch `FIRST`. VM reset MUST drain entered callbacks before clearing RX buffer/index/run state under the same synchronization, so neither a paused `FIRST` nor a closed-epoch `FIRST` can combine with a later-epoch `LAST`. A generic-response pump MUST retain the exact failed-fragment offset/index after `PBLE_TX_AGAIN` while it still owns the logical message, because the failed packet was not accepted. It MUST restart the identical encoded frame from `FIRST` only after a successful single-fragment `RUN`, `STOP`, or `SOFT_REBOOT` response changed the mutex-protected TX stream generation. — *(source: PRD §10.7, [protocol.md §3.2](../protocol.md#3-framing); verify: conformance; story: F-02, P-02)*
...
- **FR-PROTO-6** — Every admitted command MUST return the correct PBLE/1 1-byte status in its `RSP`, including error cases (`EBADREQ`, `ENOENT`, `EACCES`, `ENOSPC`, `EIO`, `ENOMEM`, `EBUSY`, `ECRC`, `ERANGE`, `EUNSUPPORTED`, `EINTERNAL`). Before invoking an ordinary synchronous side-effecting handler, the reference dispatcher MUST atomically reserve one whole encoded response in a fixed depth-2 pool; deferred filesystem commands MUST reserve a pool ticket before host-to-worker enqueue and carry its stored slot incarnation, connection generation, and VM epoch. Reserve MUST capture the epoch in the native ticket, and every match, publish, completion, and cancellation MUST compare it. Reservation failure MUST invoke no handler, enqueue no work, and make no unreserved response attempt; it MUST terminate/cancel the originating session so bounded observable link loss, not a silent live-session drop, is the refusal outcome. If a deferred filesystem enqueue is full after reservation, no filesystem operation runs and the dispatcher MUST publish `EBUSY` through that same reserved slot. When a fully encoded response enters the ready FIFO, one absolute 1000 ms publication deadline begins with no progress extension. A pre-created host callout retains the failed-fragment position on transient pressure under exclusive logical ownership; it restarts from `FIRST` only after a successful single-fragment `RUN`, `STOP`, or `SOFT_REBOOT` response changes the TX stream generation. Disconnect, VM-epoch change, or token mismatch cancels the item; deadline expiry on a still-live session MUST terminate that session. Every required termination MUST atomically change the exact connection token from `OPEN` to `CLOSING` before doing anything else; `CLOSING` MUST fail every admission/live check for commands, tickets, all TX, and specialized handlers while remaining available only for exact lifecycle cleanup. The ESP reference agent MUST arm a pre-created, task-dispatched watchdog for one non-extending absolute 2500 ms deadline before making exactly one host-context `ble_gap_terminate` call. Only return `0` or `BLE_HS_EALREADY` may await exact disconnect/reset with the watchdog armed. Any other return, watchdog-arm failure, or watchdog expiry MUST immediately invoke public non-returning `esp_restart()`; the agent MUST NOT retry termination or call `ble_hs_sched_reset`. A termination-failure GAP event MUST NOT clear `CLOSING` or cancel the watchdog. Exact disconnect/reset MUST cancel the watchdog and idempotently invalidate the token and all bound work before later advertising/admission; stale watchdog work MUST revalidate the token/deadline and cannot affect a reused handle. Completion/cancellation, including TX success/error result handling, MUST signal exactly once for the exact slot incarnation that transitions to complete; repeating either operation on an already-complete incarnation MUST be idempotent, and recycle MUST drain stale signals before reuse. Before old VM threads can be deleted, an allocation-free idempotent linker wrapper around `mp_thread_deinit` MUST use one authoritative admission/activity lock: close admission, invalidate old tokens, then mint and pass one absolute deadline exactly 2500 ms ahead through complete-CMD/exact-epoch activity drain, lifecycle-timer disarm, response-callout stop, and physical TX-mutex acquisition. Every stage MUST use only the residual; no helper may restart the budget. Timeout, counter invariant failure, lifecycle-timer disarm failure, callout-stop failure, or TX-quiescence failure MUST call non-returning `esp_restart()`. The soft-reboot callback MUST carry its armed epoch and enter/leave activity around rooted `SystemExit`/scheduler access; identify callbacks MUST revalidate epoch before GPIO changes. Exactly once per new VM, every PyBLE ESP board overlay's `MICROPY_PORT_INIT_FUNC` MUST atomically rotate the VM epoch and live connection generation with old-ticket invalidation; prevent future callout scheduling; synchronize with pool/TX ownership; hard-recycle all pool incarnations; and drain completion signals. A previously queued callout MUST revalidate the exact epoch/incarnation and touch nothing; reset MUST NOT depend on callout deinitialization or queued-event removal. Repeated agent initialization within one VM is idempotent. A final boot readiness barrier MUST reopen only after both fresh workers have entered, all handler/console wiring is safe, and auto-run admission has completed. Because worker entry requires the MicroPython GIL, the pinned main-task ready call MUST release it during one bounded absolute 2500 ms worker-entry wait and reacquire it before returning; timeout or boot-wiring failure leaves admission closed. The linker options and release map/nm build checks MUST prove that all ESP targets resolve `mp_thread_deinit` through the wrapper (including forced wrapper retention) and execute the per-`mp_init` hook, with no upstream source edit. The RUN/STOP/SOFT_REBOOT specialized exception is FR-RUN-1/5/8: each declares one pending connection-bound, single-fragment response attempt, may wait under one absolute 15 ms deadline only for the current complete-message TX-mutex boundary, and then makes exactly one local Notify submission without waiting or retrying for mbuf/controller capacity before its corresponding side effect. Boundary-deadline or submission failure MUST suppress generic fallback and the side effect. — *(source: PRD §10.8, [protocol.md §3.2](../protocol.md#3-framing), [§6](../protocol.md#6-run--stop--console), [§8](../protocol.md#8-status--error-codes-1-byte-status-in-rsp); verify: unit, conformance, build, HIL; story: F-02, P-04)*
  ...
  The wrapper mints one absolute deadline exactly 2500 ms ahead and passes it
  unchanged through activity drain, soft-reboot/identify timer disarm,
  prevention of future callout scheduling, and acquisition of the physical
  recursive TX mutex. Each step uses only the residual and no helper restarts
  the budget; an inactive or
  ...
  new completion.
  The native response ticket itself stores its reserved VM epoch. Reserve
  captures that value, and every ticket match, publish, completion, and cancel
  path compares it together with slot incarnation and connection generation.
  `pble_rsp_tx_result` follows the same transition-only rule: a TX result for an
  already-`COMPLETE` incarnation gives no second wake.
  ...
  readiness. Because both fresh `_thread` workers must acquire the MicroPython
  GIL to mark entry, the pinned main-task final-ready call MUST release the GIL
  during one absolute 2500 ms worker-entry wait, then reacquire it before
  return. Timeout leaves admission closed.
...
- **FR-RUN-5** — Only after the connection-bound, single-fragment `RSP{OK}` path has waited at most the one absolute 15 ms current-message TX-boundary budget and its single local Notify submission succeeds, `STOP` MUST interrupt the runner by raising `KeyboardInterrupt`, and MUST land promptly even against a tight loop (e.g. `while True: pass`). Before that TX attempt the ESP reference runner MUST mark the control attempt unresolved under runner synchronization and release that synchronization before TX. A reserved worker MUST NOT cross its RUN-event/user-effect pickup gate while an earlier control attempt is unresolved. It MUST wait outside the runner domain on a pre-created resolution signal, then loop and snapshot unresolved/stop together under the domain. One post-attempt runner cut MUST either publish accepted stop intent plus the worker pending exception before resolving the gate, or resolve failure without an interrupt/stop effect; both paths wake the worker after the cut. The active worker's terminal path MUST use the same signal and, without holding the GIL while waiting, commit the resolved-gate check, stop snapshot, and stopped-versus-finished transition in one runner-domain cut. A control begin that precedes that cut MUST resolve acceptance to `idle` and failure to the natural `done`/`error`; a terminal cut that precedes the control remains authoritative. A later successful RUN reservation may clear only resolved stale intent. Response-submission failure MUST leave execution unchanged and emit no generic fallback. — *(source: PRD §8.3, §10.6, §13.3, [protocol.md §6](../protocol.md#6-run--stop--console), [firmware.md §5](../firmware.md#5-runtime-rules); verify: unit, HIL; story: F-06)*
...
- **FR-RUN-7** — On every state transition (`idle` / `running` / `done` / `error`), the agent MUST create a `RUN_STATE` event only when it can atomically capture the then-current live `{connection handle, connection generation, VM epoch}`. A created event and all its retries MUST retain that exact token and MUST NOT retarget a successor; disconnect or VM-epoch change cancels old buffered work. A transition with no live session emits no event, while a later transition after reconnect MAY bind the successor. This applies to command-started and auto-run execution and does not alter the originating-session-bound `RUN` response/execution admission cut. — *(source: PRD §10.6, [protocol.md §6](../protocol.md#6-run--stop--console), [§4](../protocol.md#4-opcodes); verify: unit, conformance, HIL; story: F-04)*
- **FR-RUN-8** — On the normal path where its pre-created grace timer arms
  successfully, `SOFT_REBOOT` MUST submit an observable `RSP{OK}` before it
  ...
  SHOULD keep the BLE link where possible. The sole post-`RSP{OK}` exception is
  timer-arm failure, which MUST immediately hardware-restart via non-returning
  `esp_restart()` with admission closed rather than attempt the normal grace or
  soft reset. A response-submission failure MUST
  ...
  MUST commit the closure through teardown and MUST resolve the same runner
  pickup and terminal gate as STOP by publishing stop intent before timer arm.
  A worker whose reservation has not crossed pickup before that accepted
  response MUST perform no RUN event or user effect, and an active worker whose
  terminal cut has not completed MUST resolve to `idle`. Response-submission
  failure MUST reopen the provisional VM/FS gates
  before resolving the runner gate without stop intent, and leave the VM intact.
  After `PBLE_TX_OK`, gates MUST NOT
  ...
  handler/console wiring is safe, and auto-run admission has completed. The
  final-ready call MUST release the main task's MicroPython GIL during one
  absolute 2500 ms wait for both worker-entry flags and reacquire it before
  returning; timeout leaves admission closed. An
...
- **FR-FS-3** — Download (`FILE_GET_BEGIN` / `FILE_GET_DATA` / `FILE_GET_END`) MUST stream a file from an offset and report a whole-file CRC for the app to verify. Its deferred queue item MUST carry both a reserved response ticket with slot incarnation and an immutable `{handle, connection generation, VM epoch}` token; revalidate the whole ticket immediately before and after each raw VFS effect until response completion and before publishing worker-scratch output; and complete the matching `RSP` before emitting dependent `FILE_GET_DATA`/`FILE_GET_END` events. Completion wait recycles the response slot, so every later download VFS read/event MUST instead revalidate the item's immutable session/VM token and MUST NOT inspect or touch the recycled slot. Raw stat/open, every directory iterator step, each CRC/read/write chunk, close, remove/rmdir, mkdir, and rename MUST be individually bracketed; one pair around a whole handler or multi-operation loop is insufficient. Every physical data/end event attempt MUST bind the same exact token and serialize its final token check plus Notify with GAP lifecycle; an outer worker check alone is insufficient. Disconnect or VM reset cancels/recycles the ticket. No VFS operation/chunk may start after invalidation; an indivisible operation validly started before invalidation may finish, but its owner MUST revalidate afterward and emit, publish, or start nothing further. — *(source: PRD §8.4, §10.8, [protocol.md §5](../protocol.md#5-file-transfer-the-reliability-core); verify: unit, conformance, HIL; story: F-08)*
- **FR-FS-4** — Upload (`FILE_PUT_BEGIN` / `FILE_PUT_DATA` / `FILE_PUT_END`) MUST accept windowed chunks with a sliding window of up to `W` unacknowledged chunks (`W` from HELLO; the reference agent advertises **W=8** with receiver queue depth `W+2`). Clients read `W` from caps and adapt — "up to W" client semantics are unchanged. `FILE_PUT_DATA` queue items MUST carry exact `{handle, connection generation, VM epoch}` ownership, validate immediately before and after the VFS chunk write, and emit no ACK or further effect after invalidation. FS enqueue+insertion, dequeue-to-busy, and the non-blocking `SOFT_REBOOT` quiescence close plus idle/queue-empty decision MUST share synchronization; both host-context enqueue and quiescence MUST acquire that gate with zero wait. Gate contention makes a response-bearing command publish `RSP{EBUSY}` through its reserved ticket, drops `FILE_PUT_DATA` for retransmission, or returns `SOFT_REBOOT` `RSP{EBUSY}` without a reset effect. `worker_busy` MUST span the entire dequeued dispatch. A dequeued worker that loses that cut MUST cancel before starting a VFS operation and release any response ticket it owns. — *(source: PRD §8.4, §10.8, §13.4, [protocol.md §5](../protocol.md#5-file-transfer-the-reliability-core); verify: conformance, HIL; story: F-09, F-11; reference-agent W raised 4→8 2026-07-04 `[docs]` to lift stop-and-wait upload throughput — no wire change, §5 offset/watermark ACK is W-agnostic; RAM cost ≈ +1 kB (4 queue slots × ~250 B), within the ESP32-C3 heap floor per NFR-FP-HEAP)*
...
- **FR-FS-8** — `FILE_DELETE`, `MKDIR`, and `FILE_RENAME` MUST be supported, each returning the correct status. The native reference `MKDIR` MUST attempt the create directly; when that call reports `EEXIST`, it MUST freshly stat the same jailed path and return `OK` only for a directory or `EBADREQ` for a file. It MUST propagate every other create/stat error rather than treating an arbitrary failed pre-stat as proof that the path is absent. Deferred operations MUST validate their exact response/session/VM token immediately before and after every VFS call, including both calls in that `EEXIST` resolution; work invalidated before a call MUST perform no mutation, and work invalidated after it MUST publish no late response or event. — *(source: PRD §8.4, §10.8, [protocol.md §4](../protocol.md#4-opcodes), [§5](../protocol.md#5-file-transfer-the-reliability-core); verify: unit, conformance, HIL; story: F-09)*
...
- **FR-CON-3** — `CONSOLE_INPUT { bytes }` MUST feed bytes to a program blocked on `input()`/`sys.stdin`. On the ESP reference agent, the one native `os.dupterm` stream MUST drain the bounded input ring only when read by the runner worker; a main-REPL read MUST NOT consume that input. Each successful stream read MUST return exactly one queued byte. An empty or non-worker read MUST report non-blocking `EAGAIN`, never zero-byte EOF, and `MP_STREAM_POLL` MUST advertise readable state exactly when that worker has queued input. The HIL proof MUST execute a real blocked `input()` round trip through the candidate firmware; a host fake that synthesizes post-input console or terminal events cannot satisfy it. — *(source: PRD §8.2, §10.8, [protocol.md §6](../protocol.md#6-run--stop--console); verify: unit, HIL; story: F-07)*
...
- **FR-IDENT-3** — `IDENTIFY` MUST blink the configured identify LED for a **bounded duration** so the user can spot the physical board, and MUST do so on a **non-blocking path** that does not stall the BLE/agent task or a running user program (FR-BLE-11, FR-RUN-3). On the ESP reference path, `esp_timer_stop()` MUST NOT be treated as cancellation of an invocation already dequeued by the pinned task dispatcher. Re-arm MUST use a timer-task quiescence phase followed by a distinct activation callback: the drain callback stops any still-armed handler boundary and consumes no tick/GPIO/terminal-stop effect, and only the later callback may mint/publish/start the successor active arm without consuming a tick. Activation MUST lifecycle-enter its pending VM epoch and revalidate the same pending phase/epoch before publication; refusal cancels pending state and starts no periodic timer. Clear, reconfiguration, and VM disarm MUST set the timer phase idle and clear pending/active state. Unexpected stop/start errors MUST clear to idle, and incarnation mint MUST remain nonzero/fail closed before wrap. Every active callback MUST revalidate its exact VM epoch/incarnation before tick, GPIO, or stop effects. An older dequeued periodic, drain, or activation invocation MUST NOT adopt or affect a successor arm. — *(source: PRD §10.6, §13.3, [protocol.md §4](../protocol.md#4-opcodes), [§6](../protocol.md#6-run--stop--console); verify: unit, HIL, conformance; story: F-03)*
...
The retained-link-fact getter returns one atomic, non-consuming object with exactly the
...
3. For `esp32-4mb`, after disconnecting
   ...
   **5,000 ms** for the profile-exact DLE/PHY/connection-parameter facts in
   ...
   This remains an independent outer settlement ceiling for each
   retained-link-fact profile: each
