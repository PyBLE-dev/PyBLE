...
# 3b. Generate both native and frozen-Python identity from versions.lock
# [pyble] (BLD-12). The copied board directory already exists after prepare;
# write _version.py there so every ESP image freezes the same source-selected
# identity as RP2 without creating a second checked-in release literal.
...
echo "build.sh: agent version ${AGENT_VER:-0.0.0-dev} (${PROTO_VER:-PBLE/1}) -> $(basename "$VER_H") + pyble/_version.py"
