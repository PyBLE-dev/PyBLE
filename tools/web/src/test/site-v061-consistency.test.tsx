...
const release: FirmwareReleaseDescriptor = {
  ...
  version: "0.6.1",
  builtAt: "2026-09-11T03:59:01Z",
  hilStatus: "pending",
  releaseJson: {
    path: "/firmware/v0.6.1/release.json",
    sha256: "71f6aca6df07e31a1f54c7f70a48d82c7fe26b1c8ca0626a8ffc57c804fbb1bd",
  },
...
describe("current v0.6.1 website consistency", () => {
  it.each([HomePage, SupportPage])(
    "presents the owner-confirmed release as available",
    (Page) => {
      vi.spyOn(selection, "firmwareReleaseSelectedAtBuild").mockReturnValue(
        release,
      );
      const { container } = render(<Page />);
      expect(container).toHaveTextContent(/v0.6.1 firmware is available/i);
      expect(container).not.toHaveTextContent(
        /Protected candidate v0.6.1|installer is currently unavailable/i,
      );
    },
  );
  ...
  it("separates current tutorial guidance from historic captures", () => {
    render(<FilesPage />);
    expect(
      screen.getByRole("region", { name: "Tutorial overview" }),
    ).toHaveTextContent(/firmware 0.6.1/i);
    expect(screen.getByText(/historical screenshots/i)).toBeVisible();
    expect(
      screen.getByText(/all five official profiles use LFS2/i),
    ).toBeVisible();
  });
