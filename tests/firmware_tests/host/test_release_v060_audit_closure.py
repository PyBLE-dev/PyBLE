...
def prepare_exact_board_copy(
    ...
    for relative in (
        ...
        if not (canonical_pyble / relative).is_file():
            fixture.write(
                canonical_pyble,
                relative,
    ...
    for source in sorted(canonical_pyble.glob("*.py")):
        if source.name == "_version.py":
            continue
        fixture.write(board, "pyble/%s" % source.name, source.read_bytes())
    fixture.write(
        board,
        "pyble/_version.py",
        (
    ...
    fixture.write(
        ...
        (
            ...
            '#define PBLE_AGENT_VERSION "0.6.0"\n'
            '#define PBLE_PROTOCOL_VERSION "PBLE/1"\n'
