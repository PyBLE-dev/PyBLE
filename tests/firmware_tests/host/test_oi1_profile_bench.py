...
class GenericS3RetainedLinkFactExecutorRedTest(
        unittest.IsolatedAsyncioTestCase):
    async def test_generic_s3_is_exact_retained_profile_without_uart_authority(self):
        executor_type = getattr(
            profile_bench,
            "RetainedLinkFactHardwareExecutor",
            None,
        ...
        self.assertEqual(
            profile_bench.RETAINED_LINK_FACT_PROFILES,
            frozenset(
                (
                    "esp32-s3-n16r8",
                    "waveshare-esp32-s3-lcd-147b",
        ...
        class ResetOnly:
            def read_available(self):
                raise AssertionError("generic S3 UART bytes are not authority")
        ...
        await executor.begin_transfer_link_capture("esp32-s3-n16r8")
        self.assertTrue(executor._capture_started)
...
class EvidenceAndCliTest(unittest.TestCase):
    ...
    def test_generic_s3_run_selects_serial_reset_and_retained_executor(self):
        args = argparse.Namespace(
            mode="verify",
            profile="esp32-s3-n16r8",
            expect_chip="esp32-s3",
            address="test-address",
            reset_port="/dev/test-serial-reset",
            reset_baud=115200,
            application_bin="application.bin",
            partition_table_bin="partition-table.bin",
            operator_reset=False,
            ...
            output="observation.json",
            board_manufacturer="Espressif Systems",
            board_model="ESP32-S3 generic reference",
            module_marking="ESP32-S3-WROOM-1-N16R8",
            device_flash_capacity_bytes=16 * 1024 * 1024,
            device_psram_capacity_bytes=8 * 1024 * 1024,
            firmware_sha256=None,
